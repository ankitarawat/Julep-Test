...
class hpoo():
	...
	def __init__(self, config = None, flow = None):
		...
		a = self.session = requests.Session()
		b = self.session.url = self.config['url']
		#c = self.auth = encodestring(self.config['username']+ ":" + self.config['password'])
		#d = self.session.headers.update({'Authorization': 'Basic '+ self.auth.rstrip()})
		e = self.session.headers.update({'Accept':'application/json'})
